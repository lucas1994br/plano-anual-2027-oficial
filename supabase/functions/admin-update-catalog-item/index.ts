...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Em ambientes com esquema legado, a coluna "codigo" pode não existir em "solicitacoes".
    // Nessa situação, fazemos retry sem esse campo para não quebrar a edição do item do catálogo.
    ...
    try {
      ...
      const isCodigoMissing = msg.toLowerCase().includes('column "codigo" does not exist') ||
        msg.toLowerCase().includes("column codigo does not exist");

      if (!isCodigoMissing) {
        console.warn("Erro ao atualizar solicitacoes dependentes:", error);
        // Não quebra a requisição se falhar em solicitações
      } else {
        const fallbackPayload: Record<string, unknown> = { ...solicitacoesUpdatePayload };
        ...
        if (fallbackError) {
          console.warn("Erro ao atualizar solicitacoes dependentes sem codigo:", fallbackError);
        }
      }
    }
