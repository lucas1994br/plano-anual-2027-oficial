...
serve(async (req: Request) => {
  ...
  try {
    const { accessCode, solicitacaoId } = await req.json();
    ...
    // Admin pode deletar qualquer solicitação, inclusive em status "enviado"
    ...
    return new Response(JSON.stringify({ success: true, message: "Solicitação deletada com sucesso" }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
