...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Insere/Upsert do valor retido na diretoria
    rows.push({
      escopo: "diretoria",
    ...
    if (rows.length > 0) {
      ...
      if (insertError) {
          // Fallback para upsert caso já exista
    ...
    return new Response(JSON.stringify({ success: true, message: "Orçamento criado com sucesso" }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error) {
