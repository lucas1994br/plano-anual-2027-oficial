...
export const AdminVisaoGeral = () => {
  ...
  const { 
    ...
    chartDataTiposPorDiretoria, 
    chartDataStatusSolicitacoes,
    totaisSolicitacoes 
  } = useMemo(() => {
    ...
    const totalAquisicao = { aprovado: 0, reprovado: 0, pendente: 0, total: 0, valorTotal: 0 };
    const totalServExistente = { aprovado: 0, reprovado: 0, pendente: 0, total: 0, valorTotal: 0 };
    const totalServNovo = { aprovado: 0, reprovado: 0, pendente: 0, total: 0, valorTotal: 0 };
    ...
    const chartDataTiposPorDiretoria = diretoriaList
      .sort((a, b) => b.totalGeral - a.totalGeral)
      .slice(0, 5) // Top 5
      .map(d => ({
        name: d.sigla,
        "Aquisições": d.aquisicao.total,
        "Serviços": d.servicoNovo.total + d.servicoExistente.total
      }));

    const totalAprovados = totalAquisicao.aprovado + totalServExistente.aprovado + totalServNovo.aprovado;
    const totalPendentes = totalAquisicao.pendente + totalServExistente.pendente + totalServNovo.pendente;
    const totalReprovados = totalAquisicao.reprovado + totalServExistente.reprovado + totalServNovo.reprovado;

    const chartDataStatusSolicitacoes = [
      { name: "Aprovados", value: totalAprovados, color: "#10b981" },
      { name: "Pendentes", value: totalPendentes, color: "#f59e0b" },
      { name: "Reprovados", value: totalReprovados, color: "#ef4444" }
    ...
    return {
      kpis: {
        ...
        saldoDisponivel,
        eficienciaGeral,
        totalGeral: solicitacoesFiltradas.length + servicosFiltrados.length
      },
      chartDataDiretorias,
      chartDataStatus,
      chartDataCategory,
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="mb-6">
          <TabsTrigger value="orcado">Orçado x Realizado</TabsTrigger>
          <TabsTrigger value="solicitacoes">Dados de Solicitações</TabsTrigger>
        ...
        <TabsContent value="orcado" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <Card className="p-4 flex flex-col justify-center border-l-4 border-l-purple-500">
              <h3 className="text-sm font-medium text-slate-500 mb-2">Limite Orçamentário (Admin)</h3>
            ...
            <Card className="p-4 flex flex-col justify-center border-l-4 border-l-blue-500">
              <h3 className="text-sm font-medium text-slate-500 mb-2">Orçamento Planejado (Soma Itens)</h3>
            ...
            <Card className="p-4 flex flex-col justify-center border-l-4 border-l-amber-400">
              <h3 className="text-sm font-medium text-slate-500 mb-2">Eficiência Geral</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <Card className="p-4">
              <h3 className="text-lg font-semibold text-slate-800 mb-4">Orçamento Planejado x Realizado Diário (Mensal)</h3>
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartDataMonth} margin={{ top: 10, right: 10, left: 30, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="realizado" name="Realizado" stroke="#f59e0b" strokeWidth={2} dot={false} activeDot={{ r: 5 }} />
                    <Line type="monotone" dataKey="planejado" name="Orçamento Base" stroke="#3b82f6" strokeWidth={1.5} strokeDasharray="5 5" dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
        ...
        <TabsContent value="solicitacoes" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <Card className="p-4">
              <h3 className="text-lg font-semibold text-slate-800 mb-4">Volume por Tipo (Top 5 Diretorias)</h3>
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartDataTiposPorDiretoria} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.3} />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={12} fill="#64748b" />
                    ...
                    />
                    <Legend iconType="circle" wrapperStyle={{ fontSize: '11px', color: '#64748b' }} />
                    <Bar dataKey="Aquisições" fill="#8b5cf6" radius={[2, 2, 0, 0]} />
                    <Bar dataKey="Serviços" fill="#0ea5e9" radius={[2, 2, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card className="p-4 lg:col-span-2">
              <h3 className="text-lg font-semibold text-slate-800 mb-4">Volume Financeiro (R$) por Diretoria</h3>
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartDataVolumeGeral} margin={{ top: 10, right: 10, left: 30, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.3} />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={12} />
                    <YAxis 
                      axisLine={false} 
                    ...
                    />
                    <Legend iconType="circle" wrapperStyle={{ fontSize: '11px', color: '#64748b' }} />
                    <Bar dataKey="Aquisições" stackId="a" fill="#6366f1" />
                    <Bar dataKey="Serv. Existentes" stackId="a" fill="#10b981" />
                    <Bar dataKey="Serviços Novos" stackId="a" fill="#06b6d4" radius={[2, 2, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <div className="lg:col-span-1 flex flex-col gap-6">
              <Card className="p-4 flex-1">
                <h3 className="text-lg font-semibold text-slate-800 mb-4">Top Subcategorias</h3>
              ...
              <Card className="p-4 flex-1">
                <h3 className="text-lg font-semibold text-slate-800 mb-4">Status de Aprovação</h3>
                <div className="h-48 w-full flex items-center justify-center relative">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={chartDataStatusSolicitacoes}
                        cx="50%"
                        cy="50%"
                        innerRadius={55}
                        outerRadius={75}
                        ...
                        strokeWidth={1}
                      >
                        {chartDataStatusSolicitacoes.map((entry: any, index: number) => (
                          <Cell key={`cell-${index}`} fill={entry.color} />
                        ))}
                      </Pie>
                      <RechartsTooltip 
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    ...
                    <span className="text-xl font-bold text-slate-700">{totaisSolicitacoes.totalAprovados + totaisSolicitacoes.totalPendentes + totaisSolicitacoes.totalReprovados}</span>
                  </div>
                </div>
              </Card>
            </div>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="overflow-auto max-h-[500px]">
          <table className="w-full text-sm text-left relative">
            <thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-100 sticky top-0 z-10 shadow-sm">
              <tr>
      ...
      <TabsContent value="por-diretoria" className="space-y-6">
        {filterDiretoria !== "all" ? (
          <AdminDiretoriaDashboard 
            diretoriaId={filterDiretoria} 
          ...
          <div className="flex flex-col justify-center items-center h-48 border-2 border-dashed border-slate-200 rounded-lg text-slate-400 bg-slate-50">
            <Filter className="w-8 h-8 mb-2 opacity-50" />
            <p>Selecione uma Diretoria específica nos filtros principais acima.</p>
          </div>
        )}
      </TabsContent>
      </Tabs>
